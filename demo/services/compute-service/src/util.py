...
async def with_exponential_backoff(
    ...
    while attempt < max_attempts:
        ...
        if attempt >= max_attempts:
            logger.error(f"Failed to {operation_name} after {max_attempts} attempts: {result.error}")
            return cast(FailureResult[E], result)
            
        logger.warning(f"{operation_name} attempt {attempt} failed: {result.error}. Retrying in {backoff_time}s...")
